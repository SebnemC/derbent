...
/**
 * CGenericSearchService - Utility class providing generic search methods using reflection.
 * Layer: Service (MVC) - Utility
 * Provides common search operations for entities with standard fields like name, project, assignedTo, createdBy.
 */
public final class CGenericSearchService {
     ...
     */
    public static <T extends CEntityDB<T>> Specification<T> createFieldSpec(final String fieldName, final Object fieldValue) {
     ...
     * @param <T> the entity type extending CEntityNamed
     ...
     * @param <T> the entity type extending CEntityOfProject
     ...
     * 
     * @param service the service instance to use for searching
     * @param fieldName the name of the field to search by
     * @param fieldValue the value to search for
    ...
    public static <T extends CEntityDB<T>> List<T> findByField(
            final CAbstractService<T> service,
            final String fieldName,
            final Object fieldValue,
            final Pageable pageable) {
        
        LOGGER.debug("Generic search by field '{}' with value '{}' for {}", 
                    fieldName, fieldValue, service.getClass().getSimpleName());

        try {
            // Verify the field exists in the entity class
            if (!hasField(service.getEntityClass(), fieldName)) {
                LOGGER.warn("Field '{}' not found in entity class {}", fieldName, service.getEntityClass().getSimpleName());
            ...
            LOGGER.error("Error in generic search by field '{}' for {}: {}", 
                        fieldName, service.getClass().getSimpleName(), e.getMessage(), e);
            return new ArrayList<>();
        }
    }
    ...
    public static boolean hasField(final Class<?> entityClass, final String fieldName) {
        try {
            ...
            LOGGER.warn("Error checking field existence for '{}' in class {}: {}", 
    ...
    public static List<String> getSearchableFields(final Class<?> entityClass) {
        ...
        try {
            ...
            LOGGER.warn("Error getting searchable fields for class {}: {}", 
